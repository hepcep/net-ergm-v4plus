...
## edges
edgecount.sim.data <- (unlist(lapply(sim_results, 
    function (x) network.edgecount(x)))) #edge count summary

mean(edgecount.sim.data) # sim
...
ggplot(edgecount_df, aes(x = category, y = count)) +
    ...
    geom_hline(yintercept = edges_target, linetype = "solid", 
        color = "black", size = 1.5) + 
            scale_y_continuous(
        limits = c(24000, 27000),  
        breaks = seq(24000, 27000, by = 1000),          labels = scales::comma  # Format labels with commas
    ) +
    theme_minimal() + 
    labs(y = "Edge Count", x = "Category") +
    theme(
